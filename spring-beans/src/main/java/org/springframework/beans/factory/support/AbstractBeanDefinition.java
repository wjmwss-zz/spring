...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	public int getResolvedAutowireMode() {
		// AUTOWIRE_AUTODETECT：自动检测模式，获得对应的检测模式
	 ...
	 *                                           在 XML Document -> XML BeanDefinition 中，已经详细分析了这两个标签的用法和解析过程，知道解析过程其实就是讲这两个配置存放在 BeanDefinition 中的 methodOverrides 属性中。
	 *                                           在 bean 实例化的过程中如果检测到存在 methodOverrides ，则会动态地为当前 bean 生成代理并使用对应的拦截器为 bean 做增强处理，具体的实现我们后续分析，与该函数是有关的，现在先看 mbdToUse.prepareMethodOverrides() 代码块，都干了些什么事，代码如下：
	 */
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		// Check that lookup methods exist and determine their overloaded status.
		// 如果存在 methodOverrides ，则获取所有的 override method ，然后通过迭代的方法一次调用 #prepareMethodOverride(MethodOverride mo)
		if (hasMethodOverrides()) {
			// 循环，执行 prepareMethodOverride，具体解析见函数体内
