...
public class SimpleInstantiationStrategy implements InstantiationStrategy {
	...
	public Object instantiate(RootBeanDefinition bd, @Nullable String beanName, BeanFactory owner) {
		...
		if (!bd.hasMethodOverrides()) {
			...
			// 通过 BeanUtils 直接使用构造器对象实例化 Bean 对象（反射创建）
	 ...
	 * Instantiati on should use a no-arg constructor.
	 */
	...
	public Object instantiate(RootBeanDefinition bd, @Nullable String beanName, BeanFactory owner, final Constructor<?> ctor, Object... args) {
		// 如果该 bean 没有配置 lookup-method、replaced-method 标签或者 @Lookup 注解，则直接通过反射的方式实例化 Bean 对象即可，方便快捷，具体解析见函数体内
		if (!bd.hasMethodOverrides()) {
			if (System.getSecurityManager() != null) {
			...
			// 通过 BeanUtils 直接使用构造器对象实例化 Bean 对象（通过反射实例化，方便快捷）
			return BeanUtils.instantiateClass(ctor, args);
		} else {
			// 但是，如果存在需要覆盖的方法或者动态替换的方法时，则需要使用 CGLIB 进行动态代理，因为可以在创建代理的同时将动态方法织入类中，具体解析见函数体内
