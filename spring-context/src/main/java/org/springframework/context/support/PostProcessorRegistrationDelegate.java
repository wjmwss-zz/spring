...
final class PostProcessorRegistrationDelegate {
	 ...
	 * <p>
	 * 该函数的作用：ApplicationContext 的自动检测：
	...
	public static void registerBeanPostProcessors(ConfigurableListableBeanFactory beanFactory, AbstractApplicationContext applicationContext) {
		...
		/**
		 * 方法首先 beanFactory 获取注册到该 BeanFactory 中所有 BeanPostProcessor 类型的 beanName 数组，
		 * 其实就是找所有实现了 BeanPostProcessor 接口的 bean ，然后迭代这些 bean ，
		 * 将其按照 PriorityOrdered、Ordered、无序的顺序，添加至相应的 List 集合中，
		 * 最后依次调用
		 * #sortPostProcessors(List<?> postProcessors, ConfigurableListableBeanFactory beanFactory) 方法来进行排序处理、
		 * #registerBeanPostProcessors(ConfigurableListableBeanFactory beanFactory, List<BeanPostProcessor> postProcessors) 方法来完成注册。
		 */
	}

	/**
	 * 对 BeanPostProcessor 进行排序
	 *
